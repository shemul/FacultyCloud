...
$(document).ready(function(e) {
	...
	socket.on('item', function (data) {
		var items = data.msg;
			...
			$(".container").append("<tr id='"+items._id+"'><td class='name'>"+items.fname+"</td><td class='price'>"+items.fdept+"</td><td class='qty'>"+items.status+"</td><td><a href='#' onclick='removeData(this)' data="+items._id+" class='btnRemove'>Remove</a></td><td><a i="+i+" data="+items._id+" href='#' onclick='editData(this)'>Edit</a></td></tr>");
   ...
   socket.on('update', function (data) {
   	...
   	var id = obj[0]._id;
		...
		$("tr#"+id+" td.name").text(obj[0].fname);
		$("tr#"+id+" td.price").text(obj[0].fdept);
		$("tr#"+id+" td.qty").text(obj[0].status); 
